 ...
 */
url_combo_privilegelevel = "privilegelevel";
url_transaction = "storeprivilege";
url_transaction_update = "updateprivilege";
url_transaction_delete = "deleteprivilege"
$(function () {
    ...
    $('#ajouter').click(function () {
        ...
        getAllprivilegelevelForComboInput("", "select2-1");
    ...
    $('#add-form').submit(function (e) {
        ...
        var idprivilegelevel = $('#add-form #select2-1').val();
        if (idprivilegelevel == "0") {
            ...
            addprivilege(str_name, str_description, idprivilegelevel);
        }
    });
...
function getAllprivilegelevelForComboInput(id_privilegeselect, id_combo)
{
    $.get(url_combo_privilegelevel, function (json, textStatus)
    {
        var results = json.results;
        if (json.recordsTotal > 0)
        {
            var oneelement = "<option class='option-item-offer'  value='0'>Choisir un niveau</option>";
            $('#' + id_combo).empty().append(oneelement);
            $.each(results, function (i, value)
            {
                var select = (results[i].id == id_privilegeselect) ? "selected" : "";
...
function addprivilege(str_name, str_description, idprivilegelevel) {
    $.ajax({
        url: url_transaction, // La ressource cibl\351e
        type: 'POST', // Le type de la requ�te HTTP.
        data: 'name=' + str_name + '&description=' + str_description + '&idprivilegelevel=' + idprivilegelevel,
        dataType: 'text',
...
function deleteprivilege(idprivilege) {
    $.ajax({
        ...
        if (json.recordsTotal == "1")
        {
            ...
            if (results.length > 0)
            {
                $.each(results, function (i, value)
                {
                    ...
                    getAllprivilegelevelForComboInput(results[i].idprivilegelevel, "select2-1-2");
